...
static int check_password (int method, const struct vec *ha1, const struct digest *digest)
{
	...
	md5 (resp, ha1, &digest->nonce, &digest->nc, &digest->cnonce, &digest->qop, &vec_a2, NULL);
	DBG (("%s: uri [%.*s] expected_resp [%.*s] resp [%.*s]", "check_password", digest->uri.len, digest->uri.ptr, 32, resp, digest->resp.len, digest->resp.ptr));
...
*/
static int parse_htpasswd_line (const char *s, struct vec *user, struct vec *domain, struct vec *ha1)
{
	...
	DBG (("parse_htpasswd_line: [%.*s] [%.*s] [%.*s]", user->len, user->ptr, domain->len, domain->ptr, ha1->len, ha1->ptr));
...
static int authorize (struct conn *c, FILE * fp)
{
	...
	if (auth_vec->len > 20 && !_shttpd_strncasecmp (auth_vec->ptr, "Digest ", 7))
	{
		...
		*user_vec = digest.user;
		...
		while (fgets (line, sizeof (line), fp) != NULL)
		{
			...
			DBG (("[%.*s] [%.*s] [%.*s]", user.len, user.ptr, domain.len, domain.ptr, ha1.len, ha1.ptr));

			if (vcmp (user_vec, &user) && !memcmp (c->ctx->options[OPT_AUTH_REALM], domain.ptr, domain.len))
			{
...
void _shttpd_send_authorization_request (struct conn *c)
{
	...
	(void) _shttpd_snprintf (buf, sizeof (buf), "Unauthorized\r\n" "WWW-Authenticate: Digest qop=\"auth\", realm=\"%s\", " "nonce=\"%lu\"", c->ctx->options[OPT_AUTH_REALM], (unsigned long) _shttpd_current_time);
...
*/
int _shttpd_edit_passwords (const char *fname, const char *domain, const char *user, const char *pass)
{
	...
	if ((fp = fopen (fname, "r")) == NULL)
		...
		_shttpd_elog (E_FATAL, NULL, "Cannot open %s: %s", tmp, strerror (errno));
	...
	while (fgets (line, sizeof (line), fp) != NULL)
	{
		...
		if ((int) strlen (user) == u.len && !memcmp (user, u.ptr, u.len) && (int) strlen (domain) == d.len && !memcmp (domain, d.ptr, d.len))
		{
			found++;
			md5 (ha1, &u, &d, &p, NULL);
