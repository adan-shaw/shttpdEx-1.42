...
static int read_dir (struct stream *stream, void *buf, size_t len)
{
	...
	do
	{
		...
		(void) _shttpd_snprintf (file, sizeof (file), "%s%s%s", stream->chan.dir.path, slash, dp->d_name);
		...
		{
			if (st.st_size < 1024)
				(void) _shttpd_snprintf (size, sizeof (size), "%lu", (unsigned long) st.st_size);
			else if (st.st_size < 1024 * 1024)
				(void) _shttpd_snprintf (size, sizeof (size), "%luk", (unsigned long) (st.st_size >> 10) + 1);
			else
				(void) _shttpd_snprintf (size, sizeof (size), "%.1fM", (float) st.st_size / 1048576);
		}
		(void) strftime (mod, sizeof (mod), "%d-%b-%Y %H:%M", localtime (&st.st_mtime));

		n = _shttpd_snprintf (line, sizeof (line), "<tr><td><a href=\"%s%s%s\">%s%s</a></td>" "<td>&nbsp;%s</td><td>&nbsp;&nbsp;%s</td></tr>\n", c->uri, slash, dp->d_name, dp->d_name, S_ISDIR (st.st_mode) ? "/" : "", mod, size);
...
void _shttpd_get_dir (struct conn *c)
{
	...
	{
		c->loc.io.head = _shttpd_snprintf (c->loc.io.buf, c->loc.io.size, "HTTP/1.1 200 OK\r\n" "Connection: close\r\n" "Content-Type: text/html; charset=utf-8\r\n\r\n" "<html><head><title>Index of %s</title>" "<style>th {text-align: left;}</style></head>" "<body><h1>Index of %s</h1><pre><table cellpadding=\"0\">" "<tr><th>Name</th><th>Modified</th><th>Size</th></tr>" "<tr><td colspan=\"3\"><hr></td></tr>", c->uri, c->uri);
		io_clear (&c->rem.io);
		c->status = 200;
