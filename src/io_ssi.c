...
void shttpd_register_ssi_func (struct shttpd_ctx *ctx, const char *name, shttpd_callback_t func, void *user_data)
{
	struct ssi_func *e;
...
static void call (struct ssi *ssi, const char *name, struct shttpd_arg *arg, char *buf, int len)
{
...
static int get_path (struct conn *conn, const char *src, int src_len, char *dst, int dst_len)
{
	...
	for (vec = accepted; vec->len > 0; vec++)
		if (src_len > vec->len && !memcmp (src, vec->ptr, vec->len))
		{
			...
			if (vec->len == 6)
			{
				len = _shttpd_snprintf (dst, dst_len, "%s%c%s", root, DIRSEP, conn->uri);
				while (len > 0 && dst[len] != '/')
					len--;
				dst += len;
				dst_len -= len;
			...
			{
				len = _shttpd_snprintf (dst, dst_len, "%s%c", root, DIRSEP);
				dst += len;
				dst_len -= len;
			}
...
static void do_include (struct ssi *ssi)
{
	...
	else if (!get_path (ssi->conn, inc->buf + 13, inc->nbuf - 13, buf, sizeof (buf)))
	{
...
static void do_command (struct ssi *ssi, char *buf, size_t len, int *n)
{
	...
	for (cmd = known_ssi_commands; cmd->func != NULL; cmd++)
		if (inc->nbuf > (size_t) st.len + cmd->vec.len && !memcmp (inc->buf + st.len, cmd->vec.ptr, cmd->vec.len))
		{
...
again:
	...
	while (n + inc->nbuf < len && (ch = fgetc (inc->fp)) != EOF)
		...
		case SSI_BUF:
			...
			else if (ch == '>' && !memcmp (inc->buf + inc->nbuf - 2, "--", 2))
			{
...
void _shttpd_do_ssi (struct conn *c)
{
	...
	(void) strftime (date, sizeof (date), "%a, %d %b %Y %H:%M:%S GMT", localtime (&_shttpd_current_time));

	c->loc.io.head = c->loc.headers_len = _shttpd_snprintf (c->loc.io.buf, c->loc.io.size, "HTTP/1.1 200 OK\r\n" "Date: %s\r\n" "Content-Type: text/html\r\n" "Connection: close\r\n\r\n", date);
	...
	{
		_shttpd_send_server_error (c, 500, "Cannot allocate SSI descriptor");
	}
	else
	{
