...
static void fix_directory_separators (char *path)
{
	for (; *path != '\0'; path++)
	{
		...
		if (*path == '\\')
			while (path[1] == '\\' || path[1] == '/')
				(void) memmove (path + 1, path + 2, strlen (path + 2) + 1);
	}
}
...
static int protect_against_code_disclosure (const wchar_t *path)
{
	...
	*/
	...
	if (!(data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && wcscmp (data.cFileName, p) != 0)
		return (FALSE);
...
struct dirent *readdir (DIR * dir)
{
	...
	{
		if (!dir->result.d_name || FindNextFileW (dir->handle, &dir->info))
		{
			result = &dir->result;
...
enum ready_mode_t
{ IS_READY_FOR_READ, IS_READY_FOR_WRITE };
...
stdoutput (void *arg)
{
	...
	while (!stop && max_recv > 0 && is_socket_ready (tp->s, IS_READY_FOR_READ) && (n = recv (tp->s, buf, max_recv, 0)) > 0)
	{
...
static void stdinput (void *arg)
{
	...
	while (!stop && ReadFile (tp->hPipe, buf, sizeof (buf), &n, NULL))
	{
		...
		for (sent = 0; !stop && sent < n; sent += k)
		{
			if (is_socket_ready (tp->s, IS_READY_FOR_WRITE) && (k = send (tp->s, buf + sent, n - sent, 0)) <= 0)
			{
...
static void spawn_stdio_thread (int sock, HANDLE hPipe, void (*func) (void *), big_int_t content_len)
{
...
int _shttpd_spawn_process (struct conn *c, const char *prog, char *envblk, char *envp[], int sock, const char *dir)
{
	...
	if (interp == NULL)
	{
		...
		interp = line + 2;
		(void) _shttpd_snprintf (cmdline, sizeof (cmdline), "%s%s%s", line + 2, line[2] == '\0' ? "" : " ", prog);
	}
	...
	*/
	...
	if (CreateProcessA (NULL, cmdline, NULL, NULL, TRUE, CREATE_NEW_PROCESS_GROUP, envblk, line, &si, &pi) == 0)
	{
		_shttpd_elog (E_LOG, c, "redirect: CreateProcess(%s): %d", cmdline, ERRNO);
		return (-1);
	}
	else
	{
...
static void systray (void *arg)
{
	...
	else if ((hWnd = CreateWindow (cls.lpszClassName, "", WS_OVERLAPPEDWINDOW, 0, 0, 0, 0, NULL, NULL, NULL, arg)) == NULL)
...
int _shttpd_set_nt_service (struct shttpd_ctx *ctx, const char *action)
{
	...
	if (!strcmp (action, "install"))
	{
		...
		hService = CreateService (hSCM, SERVICE_NAME, SERVICE_NAME, SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, path, NULL, NULL, NULL, NULL, NULL);
