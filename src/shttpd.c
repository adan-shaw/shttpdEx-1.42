...
void _shttpd_stop_stream (struct stream *stream)
{
	...
	DBG (("%d %s stopped. %lu of content data, %d now in a buffer", stream->conn->rem.chan.sock, stream->io_class ? stream->io_class->name : "(null)", (unsigned long) stream->io.total, (int) io_data_len (&stream->io)));
}
...
void _shttpd_send_server_error (struct conn *c, int status, const char *reason)
{
	...
	io_clear (&c->loc.io);
	c->loc.io.head = _shttpd_snprintf (c->loc.io.buf, c->loc.io.size, "HTTP/1.1 %d %s\r\n" "Content-Type: text/plain\r\n" "Content-Length: 12\r\n" "\r\n" "Error: %03d\r\n", status, reason, status);
...
static void decide_what_to_do (struct conn *c)
{
...
#if !defined(NO_AUTH)
	if ((c->method == METHOD_PUT || c->method == METHOD_DELETE) && (c->ctx->options[OPT_AUTH_PUT] == NULL || !_shttpd_is_authorized_for_put (c)))
	{
		_shttpd_send_authorization_request (c);
	}
	else
	...
	if (c->method == METHOD_PUT)
	{
		...
		else if ((c->loc.chan.fd = _shttpd_open (path, O_WRONLY | O_BINARY | O_CREAT | O_NONBLOCK | O_TRUNC, 0644)) == -1)
		{
	...
	else if (S_ISDIR (st.st_mode) && find_index_file (c, path, sizeof (path) - 1, &st) == -1 && !IS_TRUE (c->ctx, OPT_DIR_LIST))
	{
...
static void parse_http_request (struct conn *c)
{
	...
	*/
	...
	else
	{
		...
		_shttpd_parse_headers (c->headers, (c->request + req_len) - c->headers, &c->ch);
...
static void add_socket (struct worker *worker, int sock, int is_ssl)
{
	...
	else
	{
		...
		DBG (("%s:%hu connected (socket %d)", inet_ntoa (*(struct in_addr *) &sa.u.sin.sin_addr.s_addr), ntohs (sa.u.sin.sin_port), sock));
	}
}
...
static void read_stream (struct stream *stream)
{
	...
	DBG (("read_stream (%d %s): read %d/%d/%lu bytes (errno %d)", stream->conn->rem.chan.sock, stream->io_class ? stream->io_class->name : "(null)", n, len, (unsigned long) stream->io.total, ERRNO));
...
static void write_stream (struct stream *from, struct stream *to)
{
	...
	DBG (("write_stream (%d %s): written %d/%d bytes (errno %d)", to->conn->rem.chan.sock, to->io_class ? to->io_class->name : "(null)", n, len, ERRNO));
...
static void connection_desctructor (struct llhead *lp)
{
	...
	DBG (("Disconnecting %d (%.*s)", c->rem.chan.sock, c->ch.connection.v_vec.len, c->ch.connection.v_vec.ptr));
	...
	*/
	do_close = (c->ch.connection.v_vec.len >= vec.len && !_shttpd_strncasecmp (vec.ptr, c->ch.connection.v_vec.ptr, vec.len)) || (c->major_version < 1 || (c->major_version >= 1 && c->minor_version < 1));
...
static void process_connection (struct conn *c, int remote_ready, int local_ready)
{
	...
	DBG (("loc: %d [%.*s]", (int) io_data_len (&c->loc.io), (int) io_data_len (&c->loc.io), io_data (&c->loc.io)));
	DBG (("rem: %d [%.*s]", (int) io_data_len (&c->rem.io), (int) io_data_len (&c->rem.io), io_data (&c->rem.io)));
	...
	if ((_shttpd_current_time > c->expire_time) || (c->rem.flags & FLAG_CLOSED) || ((c->loc.flags & FLAG_CLOSED) && !io_data_len (&c->loc.io)))
...
static void process_worker_sockets (struct worker *worker, fd_set * read_set)
{
	...
	{
		c = LL_ENTRY (lp, struct conn, link);
		process_connection (c, FD_ISSET (c->rem.chan.sock, read_set), c->loc.io_class != NULL && ((c->loc.flags & FLAG_ALWAYS_READY)
#if !defined(NO_CGI)
...
int shttpd_socketpair (int sp[2])
{
	...
	if ((sock = socket (AF_INET, SOCK_STREAM, 0)) != -1 && !bind (sock, (struct sockaddr *) &sa, len) && !listen (sock, 1) && !getsockname (sock, (struct sockaddr *) &sa, &len) && (sp[0] = socket (AF_INET, SOCK_STREAM, 6)) != -1 && !connect (sp[0], (struct sockaddr *) &sa, len) && (sp[1] = accept (sock, (struct sockaddr *) &sa, &len)) != -1)
	{
...
static int set_workers (struct shttpd_ctx *ctx, const char *value)
{
	...
	new_num = atoi (value);
	old_num = 0;
	LL_FOREACH (&ctx->workers, lp) old_num++;
...
static void show_cfg_page (struct shttpd_arg *arg)
{
	...
	shttpd_printf (arg, "%s", "<table border=1" "<tr><th>Option</th><th>Description</th>" "<th colspan=2>Value</th></tr>");
	...
	{
		shttpd_printf (arg, "<form method=post><tr><td>%s</td><td>%s</td>" "<input type=hidden name=o value='%s'>" "<td><input type=text name=v value='%s'></td>" "<td><input type=submit value=save></td></form></tr>", o->name, o->description, o->name, ctx->options[o->index] ? ctx->options[o->index] : "");
	}
...
static void process_command_line_arguments (struct shttpd_ctx *ctx, char *argv[])
{
	...
	char line[BUFSIZ], opt[BUFSIZ], val[BUFSIZ], path[FILENAME_MAX], *p;
	FILE *fp;
	size_t i, line_no = 0;
