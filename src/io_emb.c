...
static int do_embedded (struct stream *stream, void *buf, size_t len)
{
	...
	call_user (stream->conn, &arg, (shttpd_callback_t) stream->conn->loc.chan.emb.func.v_func);
...
void shttpd_get_http_version (struct shttpd_arg *arg, unsigned long *major, unsigned long *minor)
{
	struct conn *c = arg->priv;
...
void shttpd_register_uri (struct shttpd_ctx *ctx, const char *uri, shttpd_callback_t callback, void *data)
{
...
void shttpd_handle_error (struct shttpd_ctx *ctx, int code, shttpd_callback_t func, void *data)
{
