...
static void prepare_environment (const struct conn *c, const char *prog, struct env_block *blk)
{
	...
	addenv (blk, "REQUEST_METHOD=%s", _shttpd_known_http_methods[c->method].ptr);
	...
	{
		addenv (blk, "REMOTE_USER=%.*s", h->user.v_vec.len, h->user.v_vec.ptr);
...
int _shttpd_run_cgi (struct conn *c, const char *prog)
{
	...
	else if (_shttpd_spawn_process (c, prog, blk.buf, blk.vars, pair[1], dir))
	{
		ret = -1;
