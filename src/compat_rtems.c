...
rtems_status_code rtems_initialize_webserver (rtems_task_priority initial_priority, rtems_unsigned32 stack_size, rtems_mode initial_modes, rtems_attribute attribute_set, rtems_shttpd_init init_callback, rtems_shttpd_addpages addpages_callback, char *webroot)
{
	...
	if (args != NULL)
	{
		...
		sc = rtems_task_create (rtems_build_name ('H', 'T', 'P', 'D'), initial_priority, stack_size, initial_modes, attribute_set, &tid);
...
int my_stat (const char *path, struct stat *stp)
{
...
int my_rename (const char *path1, const char *path2)
{
...
#if !defined(NO_CGI)
int spawn_process (struct conn *c, const char *prog, char *envblk, char **envp)
{
