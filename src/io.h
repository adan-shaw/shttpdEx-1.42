...
static __inline char *io_space (struct io *io)
{
	assert (io->buf != NULL);
	assert (io->size > 0);
...
static __inline char *io_data (struct io *io)
{
	assert (io->buf != NULL);
	assert (io->size > 0);
...
static __inline size_t io_space_len (const struct io *io)
{
	assert (io->buf != NULL);
	assert (io->size > 0);
	assert (io->head <= io->size);
...
static __inline size_t io_data_len (const struct io *io)
{
	assert (io->buf != NULL);
	assert (io->size > 0);
	assert (io->head <= io->size);
...
static __inline void io_inc_tail (struct io *io, size_t n)
{
	assert (io->buf != NULL);
	assert (io->size > 0);
	assert (io->tail <= io->head);
...
static __inline void io_inc_head (struct io *io, size_t n)
{
	assert (io->buf != NULL);
	assert (io->size > 0);
	assert (io->tail <= io->head);
